...
import { Building2, MapPin, Save, ArrowLeft } from 'lucide-vue-next';

const props = defineProps({ school: Object });
const form = useForm({
    name: props.school?.name ?? '',
    district: props.school?.district ?? '',
    division: props.school?.division ?? ''
});
    ...
    <AdminLayout>
        ...
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="flex items-center gap-3">
                <Link href="/admin/schools" class="flex h-9 w-9 items-center justify-center rounded-xl bg-surface text-slate-400 transition-colors hover:bg-slate-200 hover:text-slate-600">
                    <ArrowLeft class="size-4" />
                </Link>
                <div>
                    <h1 class="text-2xl font-extrabold text-text">{{ school ? 'Edit School' : 'Create School' }}</h1>
                    <p class="mt-1 text-sm font-medium text-muted">{{ school ? 'Update school information' : 'Add a new school to the system' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-2xl">
            <DashboardCard class="in-card">
                <div class="mb-6 flex items-center gap-3 border-b border-border/60 pb-4">
                    ...
                    <h2 class="text-[15px] font-bold text-text">School Details</h2>
                </div>

                <form @submit.prevent="submit" class="space-y-5">
                    <div>
                        <label class="block text-[11px] font-bold uppercase tracking-wider text-muted mb-1.5">School Name</label>
                        <div class="relative">
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-muted mb-1.5">District</label>
                            <div class="relative">
                                <MapPin class="size-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                                <input
                        ...
                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-muted mb-1.5">Division</label>
                            <div class="relative">
                                <MapPin class="size-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                                <input
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        <Link href="/admin/schools" class="rounded-xl px-5 py-2.5 text-[13px] font-bold text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700">
                            Cancel
                        </Link>
                        <button
                            type="submit"
                        ...
                        >
                            <Save class="size-4" />
                            {{ form.processing ? 'Saving...' : 'Save School' }}
                        </button>
                    </div>
                </form>
            </DashboardCard>
