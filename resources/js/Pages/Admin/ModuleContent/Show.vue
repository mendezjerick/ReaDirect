...
import { Pencil, BookOpen, Layers } from 'lucide-vue-next';

defineProps({ activity: Object, usageCount: Number });
</script>
...
<template>
    <AdminLayout>
        ...
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h1 class="text-2xl font-extrabold text-text">{{ activity.title }}</h1>
                <p class="mt-1 flex items-center gap-1.5 text-sm font-medium text-muted">
                    <BookOpen class="size-4" />
                    {{ activity.module?.title }} &bull; used {{ usageCount }} times
                </p>
            </div>
            <Link :href="`/admin/module-content/${activity.id}/edit`" class="group inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-[13px] font-bold text-white transition-all duration-200 hover:bg-primary-dark hover:shadow-md hover:shadow-primary/20 active:scale-[0.97]">
                <Pencil class="size-4" />
                Edit Activity
            </Link>
        </div>
        ...
        <div class="grid gap-6">
            <DashboardCard class="in-card" style="--delay: 0ms">
                <div class="mb-5 flex items-center gap-3 border-b border-border/60 pb-4">
                    ...
                    <h2 class="text-[15px] font-bold text-text">Activity Details</h2>
                </div>
                <div class="rounded-xl border border-border/40 bg-surface p-1">
                    <AdminJsonViewer :value="activity" />
                </div>
            </DashboardCard>
        </div>
    </AdminLayout>
