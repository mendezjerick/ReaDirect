        ...
        return [
            ...
            'items' => $attempt->selectedItems()->orderBy('sequence')->get()->map(function ($item) use ($attempt, $matching, $similarity, $sanitizer) {
                ...
                return [
                    ...
                    'ai_gop_enabled' => data_get($response?->ai_response, 'gop_enabled'),
                    'ai_gop_available' => data_get($response?->ai_response, 'gop_available'),
                    'ai_gop_score' => data_get($response?->ai_response, 'gop_score'),
                    'ai_gop_confidence' => data_get($response?->ai_response, 'gop_confidence'),
                    'ai_gop_decision' => data_get($response?->ai_response, 'gop_decision'),
                    'ai_gop_threshold' => data_get($response?->ai_response, 'gop_threshold'),
                    'ai_gop_expected_phonemes' => data_get($response?->ai_response, 'gop_expected_phonemes'),
                    'ai_gop_observed_phonemes' => data_get($response?->ai_response, 'gop_observed_phonemes'),
                    ...
                    'reinforcement_reason' => data_get($response?->ai_response, 'reinforcement_reason'),
